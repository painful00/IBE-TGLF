...
class PoincareBall():
    ...
    def sqdist(self, p1, p2):
        c = self.c
        sqrt_c = c ** 0.5
        test = sqrt_c * self.mobius_add(self.proj(-p1), self.proj(p2), dim=-1).norm(dim=-1, p=2, keepdim=False)
    ...
    def mobius_mat(self, X, y):
        c = self.c
        sqrt_c = c ** 0.5
        y_norm = y.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)
        Xy = torch.matmul(X, y).view(-1)
        Xy_norm = Xy.norm(dim=-1, keepdim=True, p=2).clamp_min(self.min_norm)
        res_c = 1 / sqrt_c * tanh(Xy_norm / y_norm * artanh(sqrt_c * y_norm)) * Xy / Xy_norm
        return res_c
